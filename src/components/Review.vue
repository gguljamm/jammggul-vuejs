<template>
  <div id="Review" :class="isMobile ? 'mob' : ''">
    <input-review v-if="isAuth" @close-pop="closePop" @reload="getData"></input-review>
    ...
    <div class="category" :class="categoryOpen ? 'opened' : ''">
      <div class="categoryHead">
        <button @click="changeCategory('game')" :class="viewCategory === 'game' ? 'selected' : ''">
          game
        </button>
        <button
          @click="changeCategory('culture')"
          :class="viewCategory === 'culture' ? 'selected' : ''"
        >
          culture
        </button>
        <button
          @click="changeCategory('unboxing')"
          :class="viewCategory === 'unboxing' ? 'selected' : ''"
        >
          unboxing
        </button>
      </div>
      <div class="categoryContent">
        <ul>
          <li
            ...
            :class="
              selectedCategory === viewCategory && selectedNum === index + (viewSubPaging - 1) * 10
                ? 'selected'
                : ''
            "
    ...
    <div v-if="reviewInfo[selectedCategory][selectedNum]" class="reviewContent">
      ...
      <template v-for="line in reviewInfo[selectedCategory][selectedNum].content.split('\n')">
        <div v-if="line.indexOf('#img') < 0">
          {{ line ? line : '&nbsp;' }}
        </div>
        <img
          v-else
          :src="
            reviewInfo[selectedCategory][selectedNum].imgUrl[line.split('#img')[1].split('#')[0]]
          "
        />
      </template>
    </div>
...
export default {
  ...
  methods: {
    ...
    getData() {
      ...
      const dailyQuery = query(collection(db, 'review'), orderBy('date', 'desc'));
