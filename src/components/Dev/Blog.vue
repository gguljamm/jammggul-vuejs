    ...
    <div class="devWrapper">
      <div>
        ...
        <div>
          <div class="categoryBox">
            ...
            <ul class="category">
              <li
                @click="changeIndex('recent')"
                :class="selectedIndex === 'recent' ? 'selected' : ''"
              >
                RECENT 20
              </li>
              <li @click="changeIndex('it')" :class="selectedIndex === 'it' ? 'selected' : ''">
                IT <span>({{ objCount.it }})</span>
              </li>
              <li @click="changeIndex('dev')" :class="selectedIndex === 'dev' ? 'selected' : ''">
                웹개발 <span>({{ objCount.dev }})</span>
              </li>
              <li
                @click="changeIndex('hardware')"
                :class="selectedIndex === 'hardware' ? 'selected' : ''"
              >
                하드웨어 <span>({{ objCount.hardware }})</span>
              </li>
            </ul>
          </div>
        </div>
      ...
      <div class="itList" v-if="!isMobile">
        <ul>
          <transition-group name="component-fade" mode="out-in">
            <blog-content
              v-for="(list, idx) in filteredData.filter((_, index) => index % 2 === 0)"
              :key="list.id"
              :data="list"
        ...
        <ul>
          <transition-group name="component-fade" mode="out-in">
            <blog-content
              v-for="(list, idx) in filteredData.filter((_, index) => index % 2 === 1)"
              :key="list.id"
              :data="list"
              @imgPopupOpen="imgPopupOpen"
              @openEdit="openEdit"
      ...
      <div class="itList" v-else>
        <ul>
          <transition-group name="component-fade" mode="out-in">
            <blog-content
              v-for="(list, idx) in filteredData"
              :key="list.id"
              :data="list"
              @imgPopupOpen="imgPopupOpen"
              @openEdit="openEdit"
...
const getData = () => {
  ...
  const dailyQuery = query(collection(db, 'dev-blog'), orderBy('date', 'desc'));
...
@media all and (max-width: 768px) {
  #DevInfo .devWrapper .itList > ul,
  #DevInfo .devWrapper > div > div {
    width: 100%;
    float: none;
  }
